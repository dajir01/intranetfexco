...
const fetchAsignaciones = async (query = '') => {
  ...
  loadError.value = ''
  try {
    const base = '/inventario/asignaciones-productos/disponibles'
    const url = query
      ? `${base}?q=${encodeURIComponent(query)}&per_page=15`
      : `${base}?per_page=15`

    const r = await fetch(url, { headers: { Accept: 'application/json' }, credentials: 'same-origin' })
    if (!r.ok) {
      loadError.value = `Error ${r.status}: ${r.statusText}`
      asignaciones.value = []
      return
    }
    ...
    if (!contentType.includes('application/json')) {
      loadError.value = `Respuesta no es JSON (${contentType})`
      asignaciones.value = []
      return
    }
    ...
    loadError.value = `Error de red: ${e?.message || e}`
    asignaciones.value = []
  } finally {
...
const removeProduct = () => emit('removeProduct', props.id)
      ...
      <VRow>
        <VCol cols="12" md="4">
          ...
          >
            <template #no-data>
              <div class="px-4 py-2">
                <span v-if="loadError" class="text-error">{{ loadError }}</span>
                <span v-else-if="(productoSearch || '').length > 0" class="text-disabled">Sin resultados</span>
                <span v-else class="text-disabled">Escriba para buscar</span>
              </div>
            </template>
          </VAutocomplete>
        </VCol>
